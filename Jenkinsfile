    ...
    stages {
        ...
        stage('Push & Deploy') {
            steps {
                ...
                withCredentials(bindings: [aws(credentialsId: 'aws_credentials', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                    untaggedImages=sh(script: "aws ecr list-images --region sa-east-1 --repository-name rampup-backend --filter tagStatus=UNTAGGED --query 'imageIds[*]' --output json ", returnStdout: true)
                ...
                withCredentials(bindings: [file(credentialsId:'ssh_keypair', variable:'ssh_key')]){
